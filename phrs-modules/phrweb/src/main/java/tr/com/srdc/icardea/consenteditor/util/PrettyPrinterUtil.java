...
/**
 * This class "pretty prints" an XML stream to something more human-readable. It
 * duplicates the character content with some modifications to whitespace,
 * restoring line breaks and a simple pattern of indenting child elements.
 * 
 * This version of the class acts as a SAX 2.0 <code>DefaultHandler</code>, so
 * to provide the unformatted XML just pass a new instance to a SAX parser. Its
 * output is via the {@link #toString toString} method.
 * 
 * One major limitation: we gather character data for elements in a single
 * buffer, so mixed-content documents will lose a lot of data! This works best
 * with data-centric documents where elements either have single values or child
 * elements, but not both.
 * 
 * @author Will Provost
 */
/*
 * Copyright 2002-2003 by Will Provost. All rights reserved.
 */
public class PrettyPrinterUtil extends DefaultHandler {
	/**
	...
	public static String prettyPrint(byte[] content) {
		try {
			PrettyPrinterUtil pretty = new PrettyPrinterUtil();
			SAXParserFactory factory = SAXParserFactory.newInstance();
			factory.setFeature(
					"http://xml.org/sax/features/namespace-prefixes", true);
			factory.newSAXParser().parse(new ByteArrayInputStream(content),
					pretty);
	...
	public static String prettyPrint(Document doc) throws TransformerException {
		try {
			...
			TransformerFactory.newInstance().newTransformer().transform(
					new DOMSource(doc), new StreamResult(buffer));
	...
	public static class StreamAdapter extends OutputStream {
		...
		public void flushPretty() throws IOException {
			...
			finalPrinter.println(PrettyPrinterUtil.prettyPrint(out
					.toByteArray()));
			finalPrinter.close();
			out.close();
		}
	...
	public void startDocument() throws SAXException {
		output.append("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>").append(
				endLine);
	}
	...
	/**
	 * Writes the start tag for the element. Attributes are written out, one to
	 * a text line. Starts gathering character data for the element.
	 */
	public void startElement(String URI, String name, String qName,
		...
		for (int a = 0; a < length; ++a)
			output.append(endLine).append(indent).append(standardIndent)
					.append(attributes.getQName(a)).append("=\"").append(
	...
	/**
	 * Checks the {@link #currentValue} buffer to gather element content. Writes
	 * this out if it is available. Writes the element end tag.
	 */
	public void endElement(String URI, String name, String qName)
		...
		if (currentValue == null)
			...
			output.append('>').append(currentValue.toString()).append("</")
					.append(qName).append('>');
		else
			output.append("/>");
	...
	/**
	 * When the {@link #currentValue} buffer is enabled, appends character data
	 * into it, to be gathered when the element end tag is encountered.
	 */
	 ...
	 * {@link #startElement startElement} by being initialized to a new
	 * <b>StringBuffer</b>, and then read and reset to <code>null</code> in
