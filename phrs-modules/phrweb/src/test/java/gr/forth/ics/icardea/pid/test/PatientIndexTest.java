...
public class  PatientIndexTest {
	...
	throws HL7Exception, LLPException, IOException {
		...
		if (giv_name!=null)
			qpd.getField(QIP_FLD_NUM, k++).parse(PatientIndexConstants.GNAME_SEG_FLD + "^"+giv_name);
		if (sex !=null)
			qpd.getField(QIP_FLD_NUM, k++).parse(PatientIndexConstants.SEX_SEG_FLD + "^" + sex);
		if (dob != null)
			qpd.getField(QIP_FLD_NUM, k++).parse(PatientIndexConstants.DOB_SEG_FLD + "^" + dob);
